...
# The block reward of coinbaseoutput.nValue (50) SAT/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 SAT/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"saturn-cli -h\" for help or \"saturn-cli listwallets\" to see "
...
class TestSaturnCli(SaturnTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `saturn-cli getblockchaininfo`")
        ...
        # Prevent saturn-cli from using existing rpcport in conf
        ...
        assert_raises_process_error(1, "Cookie file credentials were invalid and no rpcpassword was specified.", self.nodes[0].cli(f"-rpccookiefile={self.nodes[0].datadir_path / 'saturn.conf'}").echo)
        ...
        self.log.info("Test -getinfo and saturn-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and saturn-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (SAT/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 SAT) to the default wallet.
            ...
            # Test saturn-cli -generate with -rpcwallet in multiwallet mode.
        ...
        if not self.is_ipc_compiled():
            ...
            self.log.info("Test saturn-cli -ipcconnect triggers error if not built with IPC support")
            args = [self.binary_paths.saturncli, "-ipcconnect=unix", "-getinfo"]
            result = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            assert_equal(result.stdout, "error: saturn-cli was not built with IPC support\n")
            assert_equal(result.stderr, None)
            assert_equal(result.returncode, 1)


if __name__ == '__main__':
    TestSaturnCli(__file__).main()
