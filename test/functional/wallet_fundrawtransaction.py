...
class RawTransactionsTest(SaturnTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid saturn address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate SAT/kvB)")
        node = self.nodes[3]
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (SAT/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (SAT/kvB)
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 SAT outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 SAT
        ...
        # Fund wallet with 2 outputs, 5 SAT each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 SAT each (20 SAT total)
        ...
        # Attempt to send 29 SAT from a wallet that only has 20 SAT. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 SAT payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 SAT.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 SAT)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 SAT.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 SAT (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 SAT/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # fail with insufficient funds rather than saturnd asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 SAT target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 SAT) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 SAT covered (need to fund 0.9 SAT more).
    ...
    def test_cannot_cover_fees(self):
        ...
        # Set up wallet with 2 utxos: 0.3 SAT and 0.15 SAT
        ...
        self.log.info("Attempt to send 0.45 SAT without SFFO")
        ...
        self.log.info("Send 0.45 SAT with SFFO")
        wallet.fundrawtransaction(rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.45 SAT by restricting coin selection with minconf=6")
        ...
        self.log.info("Attempt to send 0.45 SAT preselecting 0.15 SAT utxo")
        rawtx = wallet.createrawtransaction(inputs=[{"txid": txid2, "vout": vout2}], outputs=[{default_wallet.getnewaddress(): 0.45}])
        assert_raises_rpc_error(-4, amount_with_fee_err_msg.format("0.00000042"), wallet.fundrawtransaction, rawtx, options={"fee_rate":1})

        self.log.info("Send 0.45 SAT preselecting 0.15 SAT utxo with SFFO")
        wallet.fundrawtransaction(hexstring=rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.15 SAT using only the 0.15 SAT preselected utxo")
        ...
        self.log.info("Send 0.15 SAT using only the 0.15 SAT preselected utxo with SFFO")
