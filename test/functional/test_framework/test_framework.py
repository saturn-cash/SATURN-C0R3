...
TMPDIR_PREFIX = "saturn_func_test_"
...
class SaturnTestMetaClass(type):
    """Metaclass for SaturnTestFramework.

    Ensures that any attempt to register a subclass of `SaturnTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'SaturnTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("SaturnTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("SaturnTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class SaturnTestFramework(metaclass=SaturnTestMetaClass):
    """Base class for a saturn test script.

    Individual saturn test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave saturnds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use saturn-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("SaturnRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a saturnd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple saturnd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the saturnd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as saturnd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in saturn.conf
    ...
    def skip_if_no_saturnd_tracepoints(self):
        """Skip the running test if saturnd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("saturnd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_saturnd_zmq(self):
        """Skip the running test if saturnd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("saturnd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if saturn-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("saturn-wallet has not been compiled")

    def skip_if_no_saturn_tx(self):
        """Skip the running test if saturn-tx has not been compiled."""
        if not self.is_saturn_tx_compiled():
            raise SkipTest("saturn-tx has not been compiled")

    def skip_if_no_saturn_util(self):
        """Skip the running test if saturn-util has not been compiled."""
        if not self.is_saturn_util_compiled():
            raise SkipTest("saturn-util has not been compiled")

    def skip_if_no_saturn_chainstate(self):
        """Skip the running test if saturn-chainstate has not been compiled."""
        if not self.is_saturn_chainstate_compiled():
            raise SkipTest("saturn-chainstate has not been compiled")

    def skip_if_no_saturn_bench(self):
        """Skip the running test if bench_saturn has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_saturn has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if saturn-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("saturn-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_saturn was compiled."""
        return self.config["components"].getboolean("BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether saturn-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether saturn-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_saturn_tx_compiled(self):
        """Checks whether saturn-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITCOIN_TX")

    def is_saturn_util_compiled(self):
        """Checks whether saturn-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_saturn_chainstate_compiled(self):
        """Checks whether saturn-chainstate was compiled."""
