# bash programmable completion for saturn(1) wrapper
# Copyright (c) 2026-present The Saturn Core developers
...
_saturn() {
    ...
    local saturn subcmd offset

    # save and use original argument to invoke saturn for help
    ...
    case "$subcmd" in
        gui|node)
            _saturn_wrap saturnd "$offset"
            return 0
            ;;
        rpc)
            _saturn_wrap saturn-cli "$offset"
            return 0
            ;;
        tx)
            _saturn_wrap saturn-tx "$offset"
            return 0
            ;;
    esac
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                options=$($saturn help 2>&1 | awk '{ for(i=1;i<=NF;i++) if ($i~/^--/) { sub(/=.*/, "=",$i); print $i } }' )
            fi
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$($saturn help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
