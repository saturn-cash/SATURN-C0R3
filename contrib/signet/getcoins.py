...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing saturn-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='saturn-cli', help='saturn-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Saturn address to which the faucet should send')
...
parser.add_argument('saturn_cli_args', nargs='*', help='Arguments to pass on to saturn-cli (default: -signet)')

args = parser.parse_args()

if args.saturn_cli_args == []:
    args.saturn_cli_args = ['-signet']


def saturn_cli(rpc_command_and_params):
    argv = [args.cmd] + args.saturn_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = saturn_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = saturn_cli(['getnewaddress', 'faucet', 'bech32'])
