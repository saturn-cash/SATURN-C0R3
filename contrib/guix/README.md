# Bootstrappable Saturn Core Builds

This directory contains the files necessary to perform bootstrappable Saturn
Core builds.
...
To build Saturn Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Saturn Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/saturn-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/saturn-core/guix.sigs).
