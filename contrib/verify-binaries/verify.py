...
"""Script for verifying Saturn Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from saturncore.org and saturn.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/saturn-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://saturncore.org"
HOST2 = "https://saturn.org"
VERSIONPREFIX = "saturn-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Saturn community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"saturn_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by saturncore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the saturn release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes saturn.org lags behind saturncore.org.)')
    )
