...
be used in Saturn Core with the `-asmap` option.
...
to binary before being used with Saturn Core.
...
the `saturn-cli getnodeaddresses` command.
...
```
saturn-cli getnodeaddresses 0 > addrs.json
```
