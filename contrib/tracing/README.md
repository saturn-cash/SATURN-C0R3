...
Tracing (USDT) support for Saturn Core on Linux using. For more information on
USDT support in Saturn Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `saturnd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `saturnd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Saturn Core USDT documentation] on how to list available tracepoints in your
`saturnd` binary.

[Saturn Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof saturnd)
```
...
Saturn P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof saturnd)
```
...
In a different terminal, starting Saturn Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/saturnd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof saturnd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof saturnd)
```
