...
Generates a saturn.conf file in `share/examples/` by parsing the output from `saturnd --help`. This script is run during the
release process to include a saturn.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-saturn.conf.sh
```
