...
static void SetupSaturnTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " saturn-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The saturn-tx tool is used for creating and modifying saturn transactions.\n\n"
                "saturn-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded saturn transaction, or with \"-create [commands]\" to create a hex-encoded saturn transaction.\n"
                "\n"
                "Usage: saturn-tx [options] <hex-tx> [commands]\n"
                "or:    saturn-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded saturn transaction
