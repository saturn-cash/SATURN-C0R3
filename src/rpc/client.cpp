...
//! Specify whether parameter should be parsed by saturn-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make saturn-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `saturn-cli` attempt to parse the
 ...
 * `saturn-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make saturn-cli treat them as positional parameters when `-named` is used.
 * This prevents `saturn-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so saturn-cli
 ...
 *
 * For example, the command `saturn-cli -named createwallet "my=wallet"`,
