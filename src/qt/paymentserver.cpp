...
const QString BITCOIN_IPC_PREFIX("saturn:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // saturn: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click saturn: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start saturn: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling saturn: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("saturn://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'saturn://' is not a valid URI. Use 'saturn:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // saturn: URI
    {
        QUrlQuery uri((QUrl(s)));
        // normal URI
        {
            SendCoinsRecipient recipient;
            if (GUIUtil::parseSaturnURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Saturn address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
