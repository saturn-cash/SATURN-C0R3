...
#include <qt/saturnaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Saturn address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new SaturnAddressEntryValidator(parent));
    widget->setCheckValidator(new SaturnAddressCheckValidator(parent));
}
...
bool parseSaturnURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no saturn: URI
    if(!uri.isValid() || uri.scheme() != QString("saturn"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!SaturnUnits::parse(SaturnUnit::SAT, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseSaturnURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseSaturnURI(uriInstance, out);
}

QString formatSaturnURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("saturn:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SaturnUnits::format(SaturnUnit::SAT, info.amount, false, SaturnUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openSaturnConf()
{
    ...
    /* Open saturn.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Saturn.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Saturn (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Saturn (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Saturn*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "saturn.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("saturn-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a saturn.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Saturn\n";
        else
            optionFile << strprintf("Name=Saturn (%s)\n", ChainTypeToString(chain));
