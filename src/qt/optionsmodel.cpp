...
/** Call node.updateRwSetting() with Saturn 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Saturn 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/saturn/saturn/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because saturn 22.x will not try to read these.
...
//! Convert enabled/size values to saturn -prune setting.
...
//! Get pruning enabled value to show in GUI from saturn -prune setting.
...
//! Get pruning size value to show in GUI from saturn -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplaySaturnUnit")) {
        settings.setValue("DisplaySaturnUnit", QVariant::fromValue(SaturnUnit::SAT));
    }
    QVariant unit = settings.value("DisplaySaturnUnit");
    if (unit.canConvert<SaturnUnit>()) {
        m_display_saturn_unit = unit.value<SaturnUnit>();
    } else {
        m_display_saturn_unit = SaturnUnit::SAT;
        settings.setValue("DisplaySaturnUnit", QVariant::fromValue(m_display_saturn_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if saturn.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_saturn_unit);
...
{
    if (new_unit.isNull() || new_unit.value<SaturnUnit>() == m_display_saturn_unit) return;
    m_display_saturn_unit = new_unit.value<SaturnUnit>();
    QSettings settings;
    settings.setValue("DisplaySaturnUnit", QVariant::fromValue(m_display_saturn_unit));
    Q_EMIT displayUnitChanged(m_display_saturn_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/saturn/saturn/pull/8273
    ...
    // (https://github.com/saturn-core/gui/issues/567).
