...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<SaturnUnit>("SaturnUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. saturn_de.qm (shortcut "de" needs to be defined in saturn.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. saturn_de_DE.qm (shortcut "de_DE" needs to be defined in saturn.qrc)
...
static const char* qt_argv = "saturn-qt";
...
void SaturnApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the SaturnApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", SaturnGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void SaturnApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool SaturnApplication::createOptionsModel(bool resetSettings)
{
...
void SaturnApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new SaturnGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &SaturnGUI::quitRequested, this, &SaturnApplication::requestShutdown);
...
void SaturnApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void SaturnApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool SaturnApplication::baseInitialize()
{
    return node().baseInitialize();
}

void SaturnApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &SaturnApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &SaturnApplication::handleRunawayException);
    connect(this, &SaturnApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &SaturnApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void SaturnApplication::parameterSetup()
{
...
void SaturnApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void SaturnApplication::requestInitialize()
{
...
void SaturnApplication::requestShutdown()
{
...
void SaturnApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // saturn: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SaturnGUI::handlePaymentRequest);
        connect(window, &SaturnGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void SaturnApplication::handleRunawayException(const QString &message)
{
...
void SaturnApplication::handleNonFatalException(const QString& message)
{
...
WId SaturnApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool SaturnApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SaturnGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(saturn);
    Q_INIT_RESOURCE(saturn_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see saturn-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see saturn-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse saturn.conf, determine network, switch to network specific
    ...
    // saturn: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
