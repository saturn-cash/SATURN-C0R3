 ...
 *
 * Ref: https://github.com/saturn/saturn/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string SaturnGUI::DEFAULT_UIPLATFORM =
...
SaturnGUI::SaturnGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &SaturnGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SaturnGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SaturnGUI::showModalOverlay);
...
void SaturnGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Saturn address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and saturn: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &SaturnGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SaturnGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SaturnGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Saturn addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Saturn addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Saturn Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Saturn Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a saturn: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Saturn command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SaturnGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SaturnGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SaturnGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SaturnGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SaturnGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SaturnGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SaturnGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SaturnGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &SaturnGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &SaturnGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &SaturnGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SaturnGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SaturnGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SaturnGUI::showDebugWindow);
}

void SaturnGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void SaturnGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SaturnGUI::setCurrentWalletBySelectorIndex);
...
void SaturnGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SaturnGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SaturnGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SaturnGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SaturnGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SaturnGUI::enableHistoryAction(bool privacy)
{
...
void SaturnGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SaturnGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SaturnGUI::removeWallet);
...
WalletController* SaturnGUI::getWalletController()
{
    return m_wallet_controller;
}

void SaturnGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SaturnGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SaturnGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SaturnGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SaturnGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SaturnGUI::incomingTransaction);
    connect(this, &SaturnGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void SaturnGUI::removeWallet(WalletModel* walletModel)
{
...
void SaturnGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SaturnGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SaturnGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SaturnGUI::setWalletActionsEnabled(bool enabled)
{
...
void SaturnGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SaturnGUI::toggleHidden);
...
void SaturnGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SaturnGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SaturnGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SaturnGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SaturnGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SaturnGUI::openClicked()
{
...
void SaturnGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SaturnGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SaturnGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SaturnGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SaturnGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SaturnGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SaturnGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SaturnGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Saturn network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void SaturnGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SaturnGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SaturnGUI::updateHeadersSyncProgressLabel()
{
...
void SaturnGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SaturnGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SaturnGUI::quitRequested);
...
void SaturnGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SaturnGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &SaturnGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void SaturnGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SaturnGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SaturnGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SaturnGUI::show);
                e->ignore();
            }
        }
    }
...
void SaturnGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SaturnGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SaturnGUI::incomingTransaction(const QString& date, SaturnUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SaturnUnits::formatWithUnit(unit, amount, true));
...
void SaturnGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SaturnGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SaturnGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SaturnGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SaturnGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SaturnGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SaturnGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SaturnGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SaturnGUI::updateWindowTitle()
{
...
void SaturnGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SaturnGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SaturnGUI::detectShutdown()
{
...
void SaturnGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SaturnGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SaturnGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SaturnGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SaturnGUI::subscribeToCoreSignals()
{
...
void SaturnGUI::unsubscribeFromCoreSignals()
{
...
bool SaturnGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<SaturnUnit> units = SaturnUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SaturnUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, SaturnUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const SaturnUnit u : SaturnUnits::availableUnits()) {
        menu->addAction(SaturnUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(SaturnUnit newUnits)
{
    setText(SaturnUnits::longName(newUnits));
}
