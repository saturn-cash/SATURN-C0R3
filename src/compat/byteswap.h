...
#      define saturn_builtin_bswap16(x) __builtin_bswap16(x)
#    endif
#    if __has_builtin(__builtin_bswap32)
#      define saturn_builtin_bswap32(x) __builtin_bswap32(x)
#    endif
#    if __has_builtin(__builtin_bswap64)
#      define saturn_builtin_bswap64(x) __builtin_bswap64(x)
#    endif
#  elif defined(_MSC_VER)
#      define saturn_builtin_bswap16(x) _byteswap_ushort(x)
#      define saturn_builtin_bswap32(x) _byteswap_ulong(x)
#      define saturn_builtin_bswap64(x) _byteswap_uint64(x)
#  endif
#endif
...
{
#ifdef saturn_builtin_bswap16
    return saturn_builtin_bswap16(x);
#else
    return (x >> 8) | (x << 8);
#endif
}
...
{
#ifdef saturn_builtin_bswap32
    return saturn_builtin_bswap32(x);
#else
...
{
#ifdef saturn_builtin_bswap64
    return saturn_builtin_bswap64(x);
#else
