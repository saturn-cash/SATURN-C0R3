...
const char * const BITCOIN_CONF_FILENAME = "saturn.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //saturn-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Saturn
    //   new: C:\Users\Username\AppData\Local\Saturn
    // macOS: ~/Library/Application Support/Saturn
    // Unix-like: ~/.saturn
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Saturn";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Saturn";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Saturn";
#else
    // Unix-like
    return pathRet / ".saturn";
#endif
#endif
}
