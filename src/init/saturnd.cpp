...
namespace init {
namespace {
const char* EXE_NAME = "saturnd";

class SaturndInit : public interfaces::Init
{
public:
    SaturndInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::SaturndInit>(node);
}
} // namespace interfaces
