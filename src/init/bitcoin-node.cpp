...
namespace init {
namespace {
const char* EXE_NAME = "saturn-node";

class SaturnNodeInit : public interfaces::Init
{
public:
    SaturnNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
{
    auto init = std::make_unique<init::SaturnNodeInit>(node, argc > 0 ? argv[0] : "");
    // Check if saturn-node is being invoked as an IPC server. If so, then
