# CJDNS support in Saturn Core

It is possible to run Saturn Core over CJDNS, an encrypted IPv6 network that
...
done outside of Saturn Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Saturn network and individual nodes.
...
## Run Saturn Core with CJDNS

Once you are connected to the CJDNS network, the following Saturn Core
...
When enabled, this option tells Saturn Core that it is running in an
...
IPv6 local network. This helps Saturn Core perform better address management:
...
CJDNS support was added to Saturn Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `saturn-cli -addrinfo` to see the
...
## CJDNS-related information in Saturn Core

There are several ways to see your CJDNS address in Saturn Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `saturn-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `saturn-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `saturn-cli help getnodeaddresses` for details.
