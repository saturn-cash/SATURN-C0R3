Saturn Core version 0.11.0 is now available from:

  <https://saturn.org/bin/saturn-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer SAT/kB fee to be
...
For example, add the following to `saturn.conf`:
...
related to the blockchain in the saturn system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Saturn Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Saturn Core will 
...
For an example script see [saturn-submittx](https://github.com/laanwj/saturn-submittx).
...
- #5334 `cf87536` libsaturnconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new saturn-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only SaturnAmountField
- #5753 `0f44672` Add saturn logo to about screen
...
- #5809 `46bfbe7` Add saturn-cli man page
...
- #5849 `41259ca` contrib/init/saturnd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Saturn-Qt.app to Saturn-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for saturn-tx)
