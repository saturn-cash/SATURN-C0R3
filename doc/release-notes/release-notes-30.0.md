...
Saturn Core version v30.0 is now available from:

  <https://saturncore.org/bin/saturn-core-30.0/>
...
- A new `saturn` command line tool has been added to make features more discoverable
  and convenient to use. The `saturn` tool just calls other executables and does not
  implement any functionality on its own. Specifically `saturn node` is a synonym for
  `saturnd`, `saturn gui` is a synonym for `saturn-qt`, and `saturn rpc` is a synonym
  for `saturn-cli -named`. Other commands and options can be listed with `saturn help`.
  The new `saturn` command is an alternative to calling other commands directly, but it
...
- The new `saturn` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `saturn -m node -ipcbind=unix` it will
  ...
  (`saturn-node` instead of `saturnd`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/saturn/saturn/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/saturn/saturn/blob/master/doc/multiprocess.md)),
...
- The `test_saturn` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `saturn` command
  as `saturn test`. The `libexec/` directory also contains new `saturn-node` and
  `saturn-gui` binaries which support IPC features and are called through the
  `saturn` tool. In source builds only, `test_saturn-qt`, `bench_saturn`, and
  `saturn-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `saturn` command. See `saturn help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Saturn Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Saturn Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the saturn-wallet tool,
