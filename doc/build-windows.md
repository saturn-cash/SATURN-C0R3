...
Below are some notes on how to build Saturn Core for Windows.

The options known to work for building Saturn Core on Windows are:
...
Note that for WSL the Saturn Core source path MUST be somewhere in the default mount file system, for
example /usr/src/saturn, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\saturn`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/saturn
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/saturn --strip
```
