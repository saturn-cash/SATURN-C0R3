...
- CLI -addrinfo now returns the full set of known addresses. In previous versions (v22.0 - v30.0) the set of returned
  ...
  to connect to, which does not filter. Note: CLI -addrinfo now requires saturnd v26.0 or later, as it uses the
  getaddrmaninfo RPC internally. Users querying older, unmaintained node versions would need to use an older saturn-cli
  version. (#26988)
